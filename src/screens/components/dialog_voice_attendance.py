import streamlit as st
from src.screens.pipelines.voice_pipeline import process_bulk_audio
from src.screens.database.config import supabase
from datetime import datetime
import pandas as pd
from src.screens.components.dialog_attendance_results import show_attendance_results


@st.dialog("Voice Attendance")

def voice_attendance_dialog(selected_subject_id):
    try:
        st.write("Record audio of the students.ThenAI will recognise the students and mark their attendance automatically.")

        audio_data =None

        audio_data = st.audio_input("Record your voice using a short phrase")

        if st.button("Analyze audio",type="primary",width="stretch",icon="🎤"):
            with st.spinner("Processing Audio input..."):
                enrolled_res = supabase.table('subject_students').select('*,students(*)').eq('subject_id', selected_subject_id).execute()
                enrolled_students=enrolled_res.data
                if not enrolled_students:
                    st.warning("No students are enrolled in this subject.")
                    return
                
                candidates_dict={
                    s['students']['student_id']:s['students']['voice_embedding']
                    for s in enrolled_students if s['students'].get('voice_embedding')
                }
                
                if not candidates_dict:
                    st.error("No students have enrolled for voice attendance in this subject.")
                    return
                
                audio_bytes = audio_data.read()
                detected_score = process_bulk_audio(audio_bytes, candidates_dict)

                results, attendance_to_log =[],[]

                current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

                for node in enrolled_students:
                    student = node['students']
                    score = detected_score.get(student['student_id'], 0.0)
                    is_present = bool(score > 0.1)  # Assuming a threshold of 0.1 for presence
                    results.append({
                        "Name":student['name'],
                        "ID":student['student_id'],
                        "Source": score if is_present else "-",
                        "Status":"Present✅" if is_present else "❌Absent"

                    })

                    attendance_to_log.append({
                        "student_id": student['student_id'],
                        "subject_id": selected_subject_id,
                        "timestamp": current_timestamp,
                        "is_present": bool(is_present) 
                    })

                st.session_state.voice_attendance_results = (pd.DataFrame(results),attendance_to_log)

        if st.session_state.get('voice_attendance_results'):
            st.divider()
            df_results,logs =st.session_state.voice_attendance_results
            show_attendance_results(df_results,logs)
    except Exception as _e:
        import traceback
        st.error(f"Dialog error: {_e}")
        st.text(traceback.format_exc())