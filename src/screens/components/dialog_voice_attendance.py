...
import pandas as pd
from src.screens.components.dialog_attendance_results import show_attendance_results


@st.dialog("Voice Attendance")
def voice_attendance_dialog(selected_subject_id):
    try:
        st.write("Record audio of the students. Then AI will recognize the students and mark their attendance automatically.")

        audio_data = st.audio_input("Record audio phrase from students")

        if st.button("Analyze audio", type="primary", use_container_width=True, icon="🎤"):
            if not audio_data:
                st.warning("Please record audio first before analyzing.")
                return

            with st.spinner("Processing Audio input..."):
                ...
                if not candidates_dict:
                    st.error("No enrolled students have voice embeddings registered for this subject.")
                    return
