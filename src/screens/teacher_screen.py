...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout", type="secondary", key='teacher_logout_btn', shortcut="ctrl+h"):
...
def teacher_tab_take_attendance():
    ...
    with c1:
        if st.button('Clear Photos', type="tertiary", use_container_width=True, disabled=not has_photos, icon="🗑️"):
            st.session_state.attendance_images = []
            st.rerun()
...
def teacher_tab_manage_subjects():
    ...
    if subjects:
        for sub in subjects:
            ...
            def share_btn(sub_data=sub):
                if st.button(
                    f"Share Code: {sub_data['subject_code']}",
                    key=f"share_{sub_data['subject_code']}",
                    type="secondary",
...
def login_teacher(username, password):
    ...
    try:
        ...
        print(f"Error logging in teacher: {str(e)}")
...
def teacher_screen_login():
    ...
    st.header("Please login to your account")
    st.write("")
    teacher_username = st.text_input("Enter Username", placeholder="vaidehi_parshurami", key="log_t_user")
    teacher_pass = st.text_input("Enter Password", placeholder="Enter password", type="password", key="log_t_pass")
    st.divider()

    btnc1, btnc2 = st.columns(2, gap="large")
    with btnc1:
        if st.button("🔐 Login", shortcut="ctrl+enter", use_container_width=True, key="log_t_submit_btn"):
    ...
    with btnc2:
        if st.button("📝 Register Instead", type="primary", use_container_width=True, key="log_t_reg_instead_btn"):
...
def register_teacher(teacher_username, teacher_name, teacher_pass, teacher_pass_confirm):
    t_user = teacher_username.strip() if teacher_username else ""
    t_name = teacher_name.strip() if teacher_name else ""
    t_pass = teacher_pass.strip() if teacher_pass else ""
    t_confirm = teacher_pass_confirm.strip() if teacher_pass_confirm else ""

    if not t_user or not t_name or not t_pass or not t_confirm:
    ...
    try:
        ...
        if "getaddrinfo failed" in str(e) or "ConnectError" in str(e):
            return False, "Could not connect to Supabase database. Please check your network or Supabase status."
...
def teacher_screen_register():
    ...
    teacher_username = st.text_input("Enter Username", placeholder="vaidehi_parshurami", key="reg_t_user")
    teacher_name = st.text_input("Enter Name", placeholder="Vaidehi Parshurami", key="reg_t_name")
    teacher_pass = st.text_input("Enter Password", placeholder="Enter password", type="password", key="reg_t_pass")
    teacher_pass_confirm = st.text_input("Confirm Password", placeholder="Confirm password", type="password", key="reg_t_pass_confirm")
    st.divider()

    btnc1, btnc2 = st.columns(2, gap="large")
    with btnc1:
        if st.button("🔐 Login Instead", use_container_width=True, key="reg_t_login_instead_btn"):
            st.session_state.teacher_login_type = 'login'
            st.rerun()
    with btnc2:
        if st.button("📝 Register now", type="primary", shortcut="ctrl+enter", use_container_width=True, key="reg_t_submit_btn"):
